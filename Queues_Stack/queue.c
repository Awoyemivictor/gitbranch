...
int main()
{
	...
	// Try enqueing and dequeuing again to make sure that our previous
...
int enqueue(struct queue* qPtr, int val)
{
	...
	// Regualar case where our queue isn't full.
	...
	else
	{
		...
		// Enqueue the new item, nwo that there is space. WE are guaranteed that
...
Post-condition: If the queue pointed to by qPtr is non-empty, then the value
				at the front of the queue is deleted from the queue and returned. Otherwise, -1 				is returned to signify that the queue was already empty when the dequeue was 					attempted. 
				*/
